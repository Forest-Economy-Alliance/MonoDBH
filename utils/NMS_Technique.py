...
input_folder = "notebooks/branches_data"
output_folder = "notebooks/seg_experiment/branches_data_output"
os.makedirs(output_folder, exist_ok=True)
 
# Class prompt
text = "tree trunk. hand"
...
def apply_nms(boxes, scores, labels, iou_threshold=0.2):
    """Perform Non-Maximum Suppression while keeping boxes, scores, and labels aligned."""
    if len(boxes) == 0:
    ...
    keep_idx = torchvision.ops.nms(boxes_tensor, scores_tensor, iou_threshold)

    # Keep everything in sync
    boxes_nms = boxes_tensor[keep_idx].numpy()
    scores_nms = scores_tensor[keep_idx].numpy()
    labels_nms = [labels[i] for i in keep_idx]
    print("all the nms box",boxes_nms)
...
for filename in os.listdir(input_folder):
    ...
    scores = processed["scores"].cpu().numpy() if "scores" in processed else np.ones(len(input_boxes))

    # 🔹 Apply NMS to filter duplicate boxes
    ...
    for trunk in trunks:
        ...
        # Angle w.r.t horizontal (surface)
        ...
        trunk["trunk_angle_deg"] = round(float(trunk_angle_with_surface), 2)
        ...
        cv2.circle(annotated, (int(mean[0]), int(mean[1])), 4, (0, 0, 255), -1)

        # Perpendicular diameter line (green)
        ...
        # Compute center X at that Y using nearby mask pixels
        mask_row_indices = np.where((ys >= y_center - 5) & (ys <= y_center + 5))[0]
        if len(mask_row_indices) == 0:
            start_x = mean[0]
            start_y = mean[1]
        else:
            start_x = float(np.mean(xs[mask_row_indices]))
            start_y = float(y_center)
        start_point = np.array([start_x, start_y], dtype=float)

        # Walk left along -normal_vec until we exit mask
        step = 1.0
        max_steps = 2000
        ...
        for _ in range(max_steps):
            new_x = int(round(point_left[0]))
            new_y = int(round(point_left[1]))
            if (0 <= new_x < trunk_mask.shape[1] and 0 <= new_y < trunk_mask.shape[0] and trunk_mask[new_y, new_x]):
                point_left -= normal_vec * step
            else:
                break

        for _ in range(max_steps):
            new_x = int(round(point_right[0]))
            new_y = int(round(point_right[1]))
            if (0 <= new_x < trunk_mask.shape[1] and 0 <= new_y < trunk_mask.shape[0] and trunk_mask[new_y, new_x]):
        ...
        trunk["diameter_line_coords"] = {"left": [int(p1[0]), int(p1[1])], "right": [int(p2[0]), int(p2[1])]}
